...
export function getSlotNodes(slot) {
  if (slot && slot.tagName.toLowerCase() === "slot")
    return !slot.assignedNodes
      ? []
      : slot.assignedNodes().filter((node) => node.nodeType === 1); //Only element nodes are relavant
  return [];
}
...
export function validateChildren({ element, allowedChildren, tagName }) {
  ...
  const kuiTypesOnly = typesToLowerCase.filter((type) =>
    type.startsWith("kui-")
  );
  const childrenTypes = [...new Set(kuiTypesOnly)];
  const notAllowedChildren = childrenTypes.filter(
    (type) => allowedChildren.indexOf(type) === -1
  );
  if (notAllowedChildren.length > 0) {
    throw new Error(
      `${notAllowedChildren.join(", ")} can't be children of ${tagName}`
    );
  }
